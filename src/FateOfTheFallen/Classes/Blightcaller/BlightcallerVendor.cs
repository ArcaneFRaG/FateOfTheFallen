    ...
    internal static class BlightcallerVendor
    {
            ...
            GameObject template)
        {
            ...
            try
            {
                ...
                /*
                 * Set VendorDesc BEFORE configuring the NPC identity.
                 * This prevents the native nameplate from retaining the
                 * cloned vendor's original merchant descriptor.
                 */
                ...
                Plugin.ModLog.Info(
                    "Kaelith the Blighted spawned successfully."
                );
            }
            catch (Exception ex)
            {
            ...
            GameObject template)
        {
            ...
            try
            {
                ...
                Plugin.ModLog.Info(
                    "Vorren the Festering spawned successfully."
                );
            }
            catch (Exception ex)
            {
            ...
            GameObject template)
        {
            ...
            try
            {
                ...
                Plugin.ModLog.Info(
                    "Veyra the Withered spawned successfully."
                );
            }
            catch (Exception ex)
            {
            ...
            Quaternion rotation)
        {
            ...
            /*
             * Disable the roaming/patrol behaviour while preserving:
             *
             * NPC
             * Character
             * NPCDialogManager
             * VendorInventory
             *
             * We deliberately do not reference NavMeshAgent because
             * this project does not currently reference UnityEngine.AIModule.
             */
            ...
            string vendorName)
        {
            ...
            /*
             * Keep the GameObject name synchronized with the custom NPC name.
             */
            vendor.name =
                vendorName;

            /*
             * Update the native NPC identity.
             *
             * VendorDesc has already been changed before this method
             * is called, so the native NPC can use the correct vendor
             * descriptor rather than inheriting the cloned descriptor.
             */
            NPC npc =
            ...
            string vendorName)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                try
                {
                    ...
                    Plugin.ModLog.Info(
                        "Blightcaller vendor identity updated: " +
                        field.Name +
                        " = " +
                        vendorName
                    );
            ...
            /*
             * Then look for writable properties.
             */
        ...
        [HarmonyPostfix]
        private static void GenericHailPostfix(
